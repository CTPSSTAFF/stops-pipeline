...
class ReportGenerator:
    ...
    def _plot_10_01_per_route_comparison(self):
        """
        Generates grouped bar charts for Table 10.01 per-route boardings, one chart per mode.
        The x-axis is the modes, and the bars are the years/scenarios.
        """
        if self.per_route_10_01_df.empty:
            return

        modes_to_plot = ['ALL', 'KNR', 'PNR', 'WLK']
        for mode in modes_to_plot:
            df_mode = self.per_route_10_01_df[self.per_route_10_01_df['Mode'] == mode]
            
            if not df_mode.empty:
                # Pivot for plotting: routes as index, years as columns
                pivot_df = df_mode.pivot_table(index='Route_ID', columns='Year', values='Boardings')
                # Exclude the 'Total' row from the chart
                ...
                if not pivot_df.empty:
                    ...
                    pivot_df.plot(
                        kind='bar',
                        figsize=(15, 8),
                        title=f'10.01 Per-Route Boardings for Mode: {mode}',
                    ...
                    plt.tight_layout()
                    plt.savefig(self.output_dir / f'10_01_per_route_{mode.lower()}_comparison.png')
                    plt.close()
    ...
    def generate_report(self):
        ...
        All other plot generation has been disabled.
        """
        ...
        # New plotting methods for the requested format
        self._plot_10_01_per_route_comparison()
